...
KE_particle = npfloat(1e6)            # eV
B_0 = npfloat(-3.12e-5)                # T
mass=m_p                               # m_e or m_p


# ===== Number of Steps  =====
rk4_step = npfloat(.4)               # .01 is good start
ps_step=rk4_step/scale_time            # set equal for one-to-one comparison, can be anything

# ===== Initial Conditions (Position & Velocity) =====
x_initial = npfloat(6)                 # all in RE
y_initial = npfloat(0)
z_initial = npfloat(.5)
pitch_deg = npfloat(15.0)              # equatorial pitch angle, 30-60 for good bounce
