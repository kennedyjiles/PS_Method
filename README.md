...
python dipoleB.py 
```
Short demo of 100 keV proton in Earth's dipole magnetic field located at a distance of 5 Earth Radii. Expected outputs are saved under `outputs_dipoleB_demo/` with trajectory and error summaries.

Reproduce full paper dataset:
```bash
python dipoleB.py paper2
```
Full simulation of a 100 keV proton in Earth's dipole magnetic fieldl located at a distance of 5 Earth Radii, exhibiting charactistic bounce and drift motions. Expected outputs are saved under `outputs_dipoleB_paper/` with trajectory and error summaries.
